...
// @updateURL   https://openuserjs.org/meta/vanowm/Airdates.tv_show_past_weeks.meta.js
...
/*Search field overlaps text below*//*
...
/*Watched*//*
body:not(.enableWatched) div.entry > input
{
	display: none;
}
...
//				MONKEY = encodeURIComponent( title ),
		...
		$.each( _engines, function( i, engine )
		{
			...
			window.open(href, MONKEY + engine.name);
		} );
	};
	parent.each(func);
}
...
function createCheckbox(id, label, checked, callback)
{
	...
	let func = function(e)
	{
		...
		return (typeof(callback) == "function") ? callback(e) : e;
	};
...
window.DB.setColor = function(id, c)
{
	if (c == "#FFFFFF")
		c = "";
	...
	if (hidden)
	{
 		if (!css.length)
 		{
			let style = "<style id='css" + id + "'>"+
									"div.calendar:not(.showHidden).activeOnly .entry[data-series-id='" + id + "'][opened]{ display: block !important; }"+
